...
v_buff_size Utils::calcEscapedStringSize(const char* data, v_buff_size size, v_buff_size& safeSize, v_uint32 flags) {
  ...
  while (i < size) {
    ...
    if(a < 32) {
      ...
      v_buff_size charSize = booster::encoding::Unicode::getUtf8CharSequenceLength(a);
      if(charSize != 0) {
        if(i + charSize > size) {
          safeSize = i;
        }
...
v_buff_size Utils::escapeUtf8Char(const char* sequence, p_char8 buffer){
  v_buff_size length;
  v_int32 code = booster::encoding::Unicode::encodeUtf8Char(sequence, length);
  if(code < 0x00010000) {
    buffer[0] = '\\';
    buffer[1] = 'u';
    booster::encoding::Hex::writeUInt16(v_uint16(code), &buffer[2]);
    return 6;
  } else if(code < 0x00200000) {
    v_int16 high;
    v_int16 low;
    booster::encoding::Unicode::codeToUtf16SurrogatePair(code, high, low);
    buffer[0] = '\\';
    buffer[1] = 'u';
    booster::encoding::Hex::writeUInt16(static_cast<v_uint16>(high), &buffer[2]);
    buffer[6] = '\\';
    buffer[7] = 'u';
    booster::encoding::Hex::writeUInt16(static_cast<v_uint16>(low), &buffer[8]);
    return 12;
  } else {
    buffer[0] = '\\';
    buffer[1] = 'u';
    buffer[2] = '+';
    booster::encoding::Hex::writeUInt32(static_cast<v_uint32>(code), &buffer[2]);
    return 11;
  }
}

booster::String Utils::escapeString(const char* data, v_buff_size size, v_uint32 flags) {
      ...
      if (a < 32) {
        ...
        switch (a) {
          ...
          default:
            resultData[pos] = '\\';
            resultData[pos + 1] = 'u';
            booster::encoding::Hex::writeUInt16(a, &resultData[pos + 2]);
            pos += 6;
            break;
...
booster::String Utils::unescapeString(const char* data, v_buff_size size, v_int64& errorCode, v_buff_size& errorPosition) {
...
const char* Utils::preparseString(ParsingCaret& caret, v_buff_size& size){
  ...
  if(caret.canContinueAtChar('"', 1)){
    ...
    caret.setError("[booster::json::Utils::preparseString()]: Error. '\"' - expected", ERROR_CODE_PARSER_QUOTE_EXPECTED);
  } else {
    caret.setError("[booster::json::Utils::preparseString()]: Error. '\"' - expected", ERROR_CODE_PARSER_QUOTE_EXPECTED);
  }
...
booster::String Utils::parseString(ParsingCaret& caret) {
  ...
  if(data != nullptr) {
    ...
    if(errorCode != 0){
      caret.setError("[booster::json::Utils::parseString()]: Error. Call to unescapeString() failed", errorCode);
...
std::string Utils::parseStringToStdString(ParsingCaret& caret){
  ...
  if(data != nullptr) {
    ...
    if(errorCode != 0){
      caret.setError("[booster::json::Utils::parseStringToStdString()]: Error. Call to unescapeStringToStdString() failed", errorCode);
      caret.setPosition(pos + errorPosition);
    } else {
      caret.setPosition(pos + size + 1);
    }
