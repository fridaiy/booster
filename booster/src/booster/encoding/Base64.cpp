...
booster::String Base64::encode(const void* data, v_buff_size size, const char* alphabet) {
  
  auto resultSize = calcEncodedStringSize(size);
  
  auto result = booster::String(resultSize);
...
booster::String Base64::encode(const booster::String& data, const char* alphabet) {
  return encode(data->data(), static_cast<v_buff_size>(data->size()), alphabet);
}
  
booster::String Base64::decode(const char* data, v_buff_size size, const char* auxiliaryChars) {
...
booster::String Base64::decode(const booster::String& data, const char* auxiliaryChars) {
