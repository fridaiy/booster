...
// Caret::Label
  ...
  booster::String Caret::Label::toString(){
    v_buff_size end = m_end;
    if(end == -1){
      end = m_caret->m_pos;
    }
    return booster::String(&m_caret->m_data[m_start], end - m_start);
  }
...
// Caret
  ...
  Caret::Caret(const booster::String& str)
  ...
  std::shared_ptr<Caret> Caret::createShared(const booster::String& str){
