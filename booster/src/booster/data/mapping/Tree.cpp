...
const type::String& Tree::Attributes::operator [] (const type::String& key) const {
  ...
  throw std::runtime_error("[booster::data::mapping::Tree::Attributes::operator []]: const operator[] can't add items.");
}
...
std::pair<type::String, std::reference_wrapper<type::String>> Tree::Attributes::operator [] (v_uint64 index) {
  ...
  throw std::runtime_error("[booster::data::mapping::Tree::Attributes::operator []]: const operator[] can't get item - empty attributes.");
}
  ...
  switch (other.m_type) {
    ...
    case Type::STRING: {
      ...
      if(otherData == nullptr) {
        throw std::runtime_error("[booster::data::mapping::Tree::setCopy()]: other.data is null, other.type is 'STRING'");
      }
    ...
    case Type::VECTOR: {
      ...
      if(otherData == nullptr) {
        throw std::runtime_error("[booster::data::mapping::Tree::setCopy()]: other.data is null, other.type is 'VECTOR'");
      }
    ...
    case Type::MAP: {
      ...
      if(otherData == nullptr) {
        throw std::runtime_error("[booster::data::mapping::Tree::setCopy()]: other.data is null, other.type is 'MAP'");
      }
    ...
    case Type::PAIRS: {
      ...
      if(otherData == nullptr) {
        throw std::runtime_error("[booster::data::mapping::Tree::setCopy()]: other.data is null, other.type is 'PAIRS'");
      }
...
v_int64 Tree::getInteger() const {
  if(m_type != Type::INTEGER) {
    throw std::runtime_error("[booster::data::mapping::Tree::getInteger()]: NOT an arbitrary INTEGER.");
  }
  v_int64 result;
...
v_float64 Tree::getFloat() const {
  if(m_type != Type::FLOAT) {
    throw std::runtime_error("[booster::data::mapping::Tree::getFloat()]: NOT an arbitrary FLOAT.");
  }
  v_float64 result;
  std::memcpy (&result, &m_data, sizeof(v_float64));
  return result;
...
const type::String& Tree::getString() const {
  if(m_type != Type::STRING) {
    throw std::runtime_error("[booster::data::mapping::Tree::getString()]: NOT a STRING.");
  }
...
const std::vector<Tree>& Tree::getVector() const {
  if(m_type != Type::VECTOR) {
    throw std::runtime_error("[booster::data::mapping::Tree::getVector()]: NOT a VECTOR.");
  }
...
const TreeMap& Tree::getMap() const {
  if(m_type != Type::MAP) {
    throw std::runtime_error("[booster::data::mapping::Tree::getMap()]: NOT a MAP.");
  }
...
const std::vector<std::pair<type::String, Tree>>& Tree::getPairs() const {
  if(m_type != Type::PAIRS) {
    throw std::runtime_error("[booster::data::mapping::Tree::getPairs()]: NOT a PAIRS.");
  }
...
std::vector<std::pair<type::String, Tree>>& Tree::getPairs() {
  ...
  if(m_type != Type::PAIRS) {
    throw std::runtime_error("[booster::data::mapping::Tree::getMap()]: NOT a PAIRS.");
  }
  auto data = reinterpret_cast<std::vector<std::pair<type::String, Tree>>*>(m_data);
  return *data;
}
...
const Tree& TreeMap::operator [] (const type::String& key) const {
  auto it = m_map.find(key);
  if(it == m_map.end()) {
    throw std::runtime_error("[booster::data::mapping::Tree::TreeMap::operator[]]: const operator[] can't add items.");
  }
  return it->second;
}
  ...
  , m_pairs(nullptr)
{
  ...
  if(tree.getType() == Tree::Type::VECTOR) {
    ...
    throw std::runtime_error("[booster::data::mapping::TreeChildrenOperator::TreeChildrenOperator()]: Node type is NOT suppoerted");
  }
}

std::pair<type::String, Tree*> TreeChildrenOperator::getPair(v_uint64 index) {
  if(m_const) {
    throw std::runtime_error("[booster::data::mapping::TreeChildrenOperator::getPair()]: Can't operate on CONST tree node");
  }
  switch (m_type) {
    case VECTOR: break;
    case MAP: {
  ...
  throw std::runtime_error("[booster::data::mapping::TreeChildrenOperator::getPair()]: Node type doesn't support pairs");
}
...
Tree* TreeChildrenOperator::getItem(v_uint64 index) {
  if(m_const) {
    throw std::runtime_error("[booster::data::mapping::TreeChildrenOperator::getItem()]: Can't operate on CONST tree node");
  }
  switch (m_type) {
  ...
  throw std::runtime_error("[booster::data::mapping::TreeChildrenOperator::getItem()]: Invalid iterator type");
}
...
const Tree* TreeChildrenOperator::getItem(v_uint64 index) const {
  ...
  throw std::runtime_error("[booster::data::mapping::TreeChildrenOperator::getItem()]: Invalid operator type");
}

Tree* TreeChildrenOperator::putPair(const type::String& key, const Tree& tree) {
  if(m_const) {
    throw std::runtime_error("[booster::data::mapping::TreeChildrenOperator::putPair()]: Can't operate on CONST tree node");
  }
  switch (m_type) {
    case VECTOR: break;
    case MAP: {
  ...
  throw std::runtime_error("[booster::data::mapping::TreeChildrenOperator::putPair()]: Node type doesn't support pairs");
}
...
Tree* TreeChildrenOperator::putItem(const Tree& tree) {
  if(m_const) {
    throw std::runtime_error("[booster::data::mapping::TreeChildrenOperator::putItem()]: Can't operate on CONST tree node");
  }
  switch (m_type) {
    case VECTOR: {
  ...
  throw std::runtime_error("[booster::data::mapping::TreeChildrenOperator::putItem()]: Invalid iterator type");
}
...
v_uint64 TreeChildrenOperator::size() const {
  ...
  throw std::runtime_error("[booster::data::mapping::TreeChildrenOperator::size()]: Invalid operator type");
}
