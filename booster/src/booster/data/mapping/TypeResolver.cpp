...
bool TypeResolver::isKnownType(const booster::Type* type) const {
  if (type != nullptr) {
...
const booster::Type* TypeResolver::resolveType(const booster::Type* type, Cache& cache) const {
...
const booster::Type* TypeResolver::findPropertyType(const booster::Type* baseType,
                                                 ...
                                                 Cache& cache) const
{
  ...
  if(pathPosition > path.size()) {
    throw std::runtime_error("[booster::data::mapping::TypeResolver::findPropertyType()]: Error. Invalid state.");
  }
                                           ...
                                           Cache& cache) const
{
  ...
  if(pathPosition > path.size()) {
    throw std::runtime_error("[booster::data::mapping::TypeResolver::findPropertyValue()]: Error. Invalid state.");
  }
...
const booster::Type* TypeResolver::resolveObjectPropertyType(const booster::Type* objectType,
                                                          const std::vector<std::string>& path,
                                                          Cache& cache) const
{
