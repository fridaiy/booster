...
v_io_size FIFOBuffer::read(void *data, v_buff_size count) {
  ...
  if(count == 0) {
    ...
    throw std::runtime_error("[booster::data::buffer::FIFOBuffer::read(...)]: count < 0");
  }
...
v_io_size FIFOBuffer::peek(void *data, v_buff_size count) {
  ...
  if(count == 0) {
    ...
    throw std::runtime_error("[booster::data::buffer::FIFOBuffer::peek(...)]: count < 0");
  }
...
v_io_size FIFOBuffer::commitReadOffset(v_buff_size count) {
  ...
  if(count == 0) {
    ...
    throw std::runtime_error("[booster::data::buffer::FIFOBuffer::commitReadOffset(...)]: count < 0");
  }
...
v_io_size FIFOBuffer::write(const void *data, v_buff_size count) {
  ...
  if(count == 0) {
    ...
    throw std::runtime_error("[booster::data::buffer::FIFOBuffer::write(...)]: count < 0");
  } else {
    m_canRead = true;
  }
...
v_io_size FIFOBuffer::readAndWriteToStream(data::stream::WriteCallback* stream, v_buff_size count, async::Action& action) {
  ...
  if(count == 0) {
    ...
    throw std::runtime_error("[booster::data::buffer::FIFOBuffer::readAndWriteToStream(...)]: count < 0");
  }
...
v_io_size FIFOBuffer::readFromStreamAndWrite(data::stream::ReadCallback* stream, v_buff_size count, async::Action& action) {
  ...
  if(count == 0) {
    ...
    throw std::runtime_error("[booster::data::buffer::FIFOBuffer::readFromStreamAndWrite(...)]: count < 0");
  }
...
async::CoroutineStarter FIFOBuffer::flushToStreamAsync(const std::shared_ptr<data::stream::OutputStream>& stream)
{
  ...
  class FlushCoroutine : public booster::async::Coroutine<FlushCoroutine> {
  private:
    FIFOBuffer* m_fifo;
...
void SynchronizedFIFOBuffer::setBufferPosition(v_buff_size readPosition, v_buff_size writePosition, bool canRead) {
  std::lock_guard<booster::concurrency::SpinLock> lock(m_lock);
