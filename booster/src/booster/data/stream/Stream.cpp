...
v_io_size WriteCallback::writeSimple(const void *data, v_buff_size count) {
  ...
  if(!action.isNone()) {
    BOOSTER_LOGe("[booster::data::stream::WriteCallback::writeSimple()]", "Error. writeSimple is called on a stream in Async mode.")
    throw std::runtime_error("[booster::data::stream::WriteCallback::writeSimple()]: Error. writeSimple is called on a stream in Async mode.");
  }
  return res;
}
...
v_io_size WriteCallback::writeExactSizeDataSimple(data::buffer::InlineWriteData& inlineData) {
  ...
  while(inlineData.bytesLeft > 0) {
    ...
    if(!action.isNone()) {
      BOOSTER_LOGe("[booster::data::stream::WriteCallback::writeExactSizeDataSimple()]", "Error. writeExactSizeDataSimple() is called on a stream in Async mode.")
      throw std::runtime_error("[booster::data::stream::WriteCallback::writeExactSizeDataSimple()]: Error. writeExactSizeDataSimple() is called on a stream in Async mode.");
    }
    ...
    if (res > 0) {
      ...
      switch (res) {
        ...
        case IOError::RETRY_WRITE:
          return async::Action::createActionByType(async::Action::TYPE_REPEAT);
        default:
          BOOSTER_LOGe("[booster::data::stream::writeExactSizeDataAsyncInline()]", "Error. Unknown IO result.")
          return new async::Error(
            "[booster::data::stream::writeExactSizeDataAsyncInline()]: Error. Unknown IO result.");
      }
    }
...
async::CoroutineStarter WriteCallback::writeExactSizeDataAsync(const void* data, v_buff_size size) {
  ...
  class WriteDataCoroutine : public booster::async::Coroutine<WriteDataCoroutine> {
  private:
...
v_io_size ReadCallback::readExactSizeDataSimple(data::buffer::InlineReadData& inlineData) {
  ...
  while(inlineData.bytesLeft > 0) {
    ...
    if(!action.isNone()) {
      BOOSTER_LOGe("[booster::data::stream::ReadCallback::readExactSizeDataSimple()]", "Error. readExactSizeDataSimple() is called on a stream in Async mode.")
      throw std::runtime_error("[booster::data::stream::ReadCallback::readExactSizeDataSimple()]: Error. readExactSizeDataSimple() is called on a stream in Async mode.");
    }
...
async::Action ReadCallback::readExactSizeDataAsyncInline(data::buffer::InlineReadData& inlineData, async::Action&& nextAction) {
  ...
  if(inlineData.bytesLeft > 0) {
    ...
    if (res > 0) {
      ...
      switch (res) {
        case IOError::BROKEN_PIPE:
          return new AsyncIOError("[booster::data::stream::readExactSizeDataAsyncInline()]: IOError::BROKEN_PIPE", IOError::BROKEN_PIPE);
        case IOError::ZERO_VALUE:
          break;
        case IOError::RETRY_READ:
          return async::Action::createActionByType(async::Action::TYPE_REPEAT);
        case IOError::RETRY_WRITE:
          return async::Action::createActionByType(async::Action::TYPE_REPEAT);
        default:
          BOOSTER_LOGe("[booster::data::stream::readExactSizeDataAsyncInline()]", "Error. Unknown IO result.")
          return new async::Error(
            "[booster::data::stream::readExactSizeDataAsyncInline()]: Error. Unknown IO result.");
      }
    }
...
//            break;
        ...
        case IOError::RETRY_WRITE:
          return async::Action::createActionByType(async::Action::TYPE_REPEAT);
        default:
          BOOSTER_LOGe("[booster::data::stream::readSomeDataAsyncInline()]", "Error. Unknown IO result.")
          return new async::Error(
            "[booster::data::stream::readSomeDataAsyncInline()]: Error. Unknown IO result.");
      }
    }
...
v_io_size ReadCallback::readSimple(void *data, v_buff_size count) {
  ...
  if(!action.isNone()) {
    BOOSTER_LOGe("[booster::data::stream::ReadCallback::readSimple()]", "Error. readSimple is called on a stream in Async mode.")
    throw std::runtime_error("[booster::data::stream::ReadCallback::readSimple()]: Error. readSimple is called on a stream in Async mode.");
  }
  return res;
}
                                      ...
                                      const base::ObjectHandle<data::buffer::Processor>& processor)
{
  ...
  class TransferCoroutine : public booster::async::Coroutine<TransferCoroutine> {
  private:
    base::ObjectHandle<ReadCallback> m_readCallback;
    base::ObjectHandle<WriteCallback> m_writeCallback;
    v_buff_size m_transferSize;
    base::ObjectHandle<booster::data::buffer::IOBuffer> m_buffer;
  ...
  public:
      ...
      if(m_procRes == data::buffer::Processor::Error::PROVIDE_DATA_IN && m_inData.bytesLeft == 0) {
        ...
        if (res > 0) {
          ...
          switch(res) {
            ...
            case IOError::BROKEN_PIPE:
              if(m_transferSize > 0) {
                return error<AsyncTransferError>("[booster::data::stream::transferAsync]: Error. ReadCallback. BROKEN_PIPE.");
              }
            ...
            default:
              if(m_transferSize > 0) {
                ...
                return error<AsyncTransferError>("[booster::data::stream::transferAsync]: Error. ReadCallback. Unknown IO error.");
              }
              m_inData.set(nullptr, 0);
    ...
    Action process() {
      ...
      switch(m_procRes) {
        ...
        default:
          return error<AsyncTransferError>("[booster::data::stream::transferAsync]: Error. ReadCallback. Unknown processing error.");
