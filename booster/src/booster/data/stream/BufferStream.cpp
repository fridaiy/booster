...
booster::String BufferOutputStream::toString() {
  return booster::String(reinterpret_cast<const char*>(m_data), m_position);
}
...
booster::String BufferOutputStream::getSubstring(v_buff_size pos, v_buff_size count) {
  if(pos + count <= m_position) {
    return booster::String(reinterpret_cast<const char*>(m_data + pos), count);
  } else {
    return booster::String(reinterpret_cast<const char*>(m_data + pos), m_position - pos);
  }
}

booster::v_io_size BufferOutputStream::flushToStream(OutputStream* stream) {
  return stream->writeExactSizeDataSimple(m_data, m_position);
}

booster::async::CoroutineStarter BufferOutputStream::flushToStreamAsync(const std::shared_ptr<BufferOutputStream>& _this, const std::shared_ptr<OutputStream>& stream) {

  class WriteDataCoroutine : public booster::async::Coroutine<WriteDataCoroutine> {
  private:
    std::shared_ptr<BufferOutputStream> m_this;
    std::shared_ptr<booster::data::stream::OutputStream> m_stream;
    data::buffer::InlineWriteData m_inlineData;
  public:

    WriteDataCoroutine(const std::shared_ptr<BufferOutputStream>& _this,
                       const std::shared_ptr<booster::data::stream::OutputStream>& stream)
      : m_this(_this)
      , m_stream(stream)
    {}
...
BufferInputStream::BufferInputStream(const booster::String& data, const std::shared_ptr<void>& captureData)
