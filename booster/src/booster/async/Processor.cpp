...
void Processor::popIOTask(CoroutineHandle* coroutine) {
  if(m_ioPopQueues.size() > 0) {
    ...
    throw std::runtime_error("[booster::async::Processor::popIOTasks()]: Error. Processor has no I/O workers.");
  }
}
...
void Processor::popTimerTask(CoroutineHandle* coroutine) {
  if(m_timerPopQueues.size() > 0) {
    ...
    throw std::runtime_error("[booster::async::Processor::popTimerTask()]: Error. Processor has no Timer workers.");
  }
}
...
void Processor::addCoroutine(CoroutineHandle* coroutine) {
  ...
  if(coroutine->_PP == this) {
    ...
    throw std::runtime_error("[booster::async::processor::addTask()]: Error. Attempt to schedule coroutine to wrong processor.");
  }
...
void Processor::waitForTasks() {
  ...
  std::unique_lock<booster::concurrency::SpinLock> lock(m_taskLock);
  ...
  while (m_running) {
    {
      ...
      auto now = booster::Environment::getMicroTickCount();
