...
#include "booster/async/worker/IOEventWorker.hpp"
#include "booster/async/worker/IOWorker.hpp"
#include "booster/async/worker/TimerWorker.hpp"
...
booster::async::Processor& Executor::SubmissionProcessor::getProcessor() {
  return m_processor;
}
...
void Executor::SubmissionProcessor::pushTasks(utils::FastQueue<CoroutineHandle>& tasks) {
  (void)tasks;
  throw std::runtime_error("[booster::async::Executor::SubmissionProcessor::pushTasks]: Error. This method does nothing.");
}

void Executor::SubmissionProcessor::pushOneTask(CoroutineHandle* task) {
  (void)task;
  throw std::runtime_error("[booster::async::Executor::SubmissionProcessor::pushOneTask]: Error. This method does nothing.");
}
  ...
  : m_balancer(0)
{
  ...
  switch(ioWorkerType) {
    ...
    default:
      throw std::runtime_error("[booster::async::Executor::Executor()]: Error. Unknown IO worker type.");
...
v_int32 Executor::chooseProcessorWorkersCount(v_int32 processorWorkersCount) {
  ...
  if(processorWorkersCount == VALUE_SUGGESTED) {
    return booster::concurrency::Utils::getHardwareConcurrency();
  }
  throw std::runtime_error("[booster::async::Executor::chooseProcessorWorkersCount()]: Error. Invalid processor workers count specified.");
}
...
v_int32 Executor::chooseIOWorkersCount(v_int32 processorWorkersCount, v_int32 ioWorkersCount) {
  ...
  throw std::runtime_error("[booster::async::Executor::chooseIOWorkersCount()]: Error. Invalid I/O workers count specified.");
}
...
v_int32 Executor::chooseTimerWorkersCount(v_int32 timerWorkersCount) {
  ...
  throw std::runtime_error("[booster::async::Executor::chooseTimerWorkersCount()]: Error. Invalid timer workers count specified.");
}

v_int32 Executor::chooseIOWorkerType(v_int32 ioWorkerType) {

  if(ioWorkerType == VALUE_SUGGESTED) {
#if defined(BOOSTER_IO_EVENT_INTERFACE_STUB)
