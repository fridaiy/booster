...
StatefulParser::StatefulParser(const booster::String& boundary,
...
StatefulParser::ListenerCall StatefulParser::parseNext_Boundary(data::buffer::InlineWriteData& inlineData) {
  ...
  throw std::runtime_error("[booster::web::mime::multipart::StatefulParser::parseNext_Boundary()]: Error. Invalid state.");
...
void StatefulParser::parseNext_AfterBoundary(data::buffer::InlineWriteData& inlineData) {
  ...
  if(m_currBoundaryCharIndex == 0) {
    ...
    if(data[0] == '-') {
      ...
      throw std::runtime_error("[booster::web::mime::multipart::StatefulParser::parseNext_AfterBoundary()]: Error. Invalid char.");
    }
  ...
  if(size > 1 || m_currBoundaryCharIndex == 1) {
    ...
    if (m_finishingBoundary && data[1 - m_currBoundaryCharIndex] == '-') {
      ...
      throw std::runtime_error("[booster::web::mime::multipart::StatefulParser::parseNext_AfterBoundary()]: Error. Invalid trailing char.");
    }
...
StatefulParser::ListenerCall StatefulParser::parseNext_Headers(data::buffer::InlineWriteData& inlineData) {
  ...
  for(v_buff_size i = 0; i < size; i ++) {
    ...
    if(m_headerSectionEndAccumulator == HEADERS_SECTION_END) {
      ...
      if(m_headersBuffer.getCurrentPosition() + i > m_maxPartHeadersSize) {
        throw std::runtime_error("[booster::web::mime::multipart::StatefulParser::parseNext_Headers()]: Error. Too large heades.");
      }
  ...
  if(m_headersBuffer.getCurrentPosition() + size > m_maxPartHeadersSize) {
    throw std::runtime_error("[booster::web::mime::multipart::StatefulParser::parseNext_Headers()]: Error. Headers section is too large.");
  }
  ...
  while(inlineData.bytesLeft > 0) {
    ...
    switch (m_state) {
      ...
      case STATE_DONE:
        return;
      default:
        throw std::runtime_error("[booster::web::mime::multipart::StatefulParser::parseNext()]: Error. Invalid state.");
    }
