...
#include "booster/web/protocol/http/incoming/SimpleBodyDecoder.hpp"
...
HttpProcessor::Components::Components(const std::shared_ptr<HttpRouter>& pRouter)
  : Components(pRouter,
               nullptr,
               std::make_shared<booster::web::protocol::http::incoming::SimpleBodyDecoder>(),
  ...
  try {
    try {
      ...
      if (!route) {
        ...
        throw booster::web::protocol::http::HttpError(protocol::http::Status::CODE_404, ss.toString());
      }
...
HttpProcessor::ConnectionState HttpProcessor::processNextRequest(ProcessingResources& resources) {
  ...
  booster::web::protocol::http::HttpError::Info error;
  ...
  if(connectionState == ConnectionState::DELEGATED) {
    ...
    if(handler) {
      ...
      BOOSTER_LOGw("[booster::web::server::HttpProcessor::processNextRequest()]", "Warning. ConnectionUpgradeHandler not set!")
      connectionState = ConnectionState::CLOSING;
    }
  }
...
HttpProcessor::Coroutine::Coroutine(const std::shared_ptr<Components>& components,
  ...
  , m_headersOutBuffer(std::make_shared<booster::data::stream::BufferOutputStream>(components->config->headersOutBufferInitial))
...
booster::async::Action HttpProcessor::Coroutine::onHeadersParsed(const RequestHeadersReader::Result& headersReadResult) {
...
HttpProcessor::Coroutine::Action HttpProcessor::Coroutine::onResponseFormed() {
  ...
  if(m_shouldInterceptResponse) {
    ...
    for (auto &interceptor: m_components->responseInterceptors) {
      ...
      if (!m_currentResponse) {
        throw booster::web::protocol::http::HttpError(protocol::http::Status::CODE_500,
  ...
  booster::web::protocol::http::utils::CommunicationUtils::considerConnectionState(m_currentRequest, m_currentResponse, m_connectionState);
  ...
  switch (m_connectionState) {
    ...
    case ConnectionState::DELEGATED: {
      ...
      if(handler) {
        ...
        BOOSTER_LOGw("[booster::web::server::HttpProcessor::Coroutine::onResponseFormed()]", "Warning. ConnectionUpgradeHandler not set!")
        m_connectionState = ConnectionState::CLOSING;
      }
      break;
    }
...
HttpProcessor::Coroutine::Action HttpProcessor::Coroutine::handleError(Error* error) {
  ...
  if(error) {
    ...
    if(error->is<booster::AsyncIOError>()) {
      auto aioe = static_cast<booster::AsyncIOError*>(error);
      if(aioe->getCode() == booster::IOError::BROKEN_PIPE) {
        return aioe; // do not report BROKEN_PIPE error
      }
    }

//    if(m_currentResponse) {
//      //BOOSTER_LOGe("[booster::web::server::HttpProcessor::Coroutine::handleError()]", "Unhandled error. '{}'. Dropping connection", error->what())
//      return error;
//    }
...
//    booster::web::protocol::http::HttpError httpError(protocol::http::Status::CODE_500, error->what());
//    auto eptr = std::make_exception_ptr(httpError);
//    m_currentResponse = m_components->errorHandler->handleError(eptr);
//    return yieldTo(&HttpProcessor::Coroutine::onResponseFormed);
