...
booster::String Response::getStatusDescription() const {
...
void Response::putHeader(const booster::String& key, const booster::String& value) {
  m_headers.put(key, value);
}

bool Response::putHeaderIfNotExists(const booster::String& key, const booster::String& value) {
...
void Response::putHeader_Unsafe(const booster::data::share::StringKeyLabelCI& key, const booster::data::share::StringKeyLabel& value) {
  m_headers.put(key, value);
}

bool Response::putHeaderIfNotExists_Unsafe(const booster::data::share::StringKeyLabelCI& key, const booster::data::share::StringKeyLabel& value) {
  return m_headers.putIfNotExists(key, value);
}

booster::String Response::getHeader(const booster::data::share::StringKeyLabelCI& headerName) const{
  return m_headers.get(headerName);
}

void Response::putBundleData(const booster::String& key, const booster::Void& polymorph) {
...
std::shared_ptr<booster::data::stream::InputStream> Response::getBodyStream() const {
  return m_bodyStream;
}
...
void Response::transferBodyToStream(const base::ObjectHandle<booster::data::stream::OutputStream>& toStream) const {
  m_bodyDecoder->decode(m_headers, m_bodyStream.get(), toStream.get(), m_connection.get());
}

booster::String Response::readBodyToString() const {
...
booster::async::CoroutineStarter Response::transferBodyToStreamAsync(const std::shared_ptr<booster::data::stream::OutputStream>& toStream) const {
