...
const char* const Header::Value::SERVER = "booster/" BOOSTER_VERSION;
const char* const Header::Value::USER_AGENT = "booster/" BOOSTER_VERSION;
...
booster::String Range::toString() const {
...
Range Range::parse(booster::utils::parser::Caret& caret) {
  ...
  auto start = booster::utils::Conversion::strToInt64(startLabel.getData());
  auto end = booster::utils::Conversion::strToInt64(endLabel.getData());
  return Range(unitsLabel.toString(), start, end);
  
}

Range Range::parse(const booster::String& str) {
  booster::utils::parser::Caret caret(str);
  return parse(caret);
}

booster::String ContentRange::toString() const {
  data::stream::BufferOutputStream stream(256);
  stream.writeSimple(units->data(), static_cast<v_buff_size>(units->size()));
  stream.writeSimple(" ", 1);
  stream.writeAsString(start);
...
ContentRange ContentRange::parse(booster::utils::parser::Caret& caret) {
  ...
  v_int64 start = booster::utils::Conversion::strToInt64(startLabel.getData());
  v_int64 end = booster::utils::Conversion::strToInt64(endLabel.getData());
  v_int64 size = 0;
  bool isSizeKnown = false;
  if(sizeLabel.getData()[0] != '*') {
    isSizeKnown = true;
    size = booster::utils::Conversion::strToInt64(sizeLabel.getData());
  }
...
ContentRange ContentRange::parse(const booster::String& str) {
  booster::utils::parser::Caret caret(str);
  return parse(caret);
}


booster::String HeaderValueData::getTitleParamValue(const data::share::StringKeyLabelCI& key) const {
...
booster::data::share::StringKeyLabelCI Parser::parseHeaderNameLabel(const std::shared_ptr<std::string>& headersText,
  ...
  for(v_buff_size i = caret.getPosition(); i < caret.getDataSize(); i++) {
    ...
    if(a == ':' || a == ' '){
      booster::data::share::StringKeyLabelCI label(headersText, &data[caret.getPosition()], i - caret.getPosition());
      caret.setPosition(i);
      return label;
      
    }
  }
  return booster::data::share::StringKeyLabelCI(nullptr, nullptr, 0);
}
...
void Parser::parseRequestStartingLine(RequestStartingLine& line,
  ...
  if(caret.findChar(' ')){
    line.method = booster::data::share::StringKeyLabel(headersText, methodLabel.getData(), methodLabel.getSize());
    caret.inc();
  } else {
  ...
  if(caret.findChar(' ')){
    line.path = booster::data::share::StringKeyLabel(headersText, pathLabel.getData(), pathLabel.getSize());
    caret.inc();
  } else {
    error = Status::CODE_400;
    return;
  ...
  if(caret.findRN()){
    line.protocol = booster::data::share::StringKeyLabel(headersText, protocolLabel.getData(), protocolLabel.getSize());
    caret.skipRN();
  } else {
    error = Status::CODE_400;
    return;
...
void Parser::parseResponseStartingLine(ResponseStartingLine& line,
  ...
  if(caret.findRN()){
    line.description = booster::data::share::StringKeyLabel(headersText, descriptionLabel.getData(), descriptionLabel.getSize());
    caret.skipRN();
  } else {
    error = Status::CODE_400;
    return;
                            ...
                            Status& error)
{
  ...
  if(name.getData() != nullptr) {
    ...
    caret.findRN();
    headers.put_LockFree(name, booster::data::share::StringKeyLabel(headersText, &caret.getData()[valuePos0], caret.getPosition() - valuePos0));
    caret.skipRN();
  } else {
...
void Parser::parseHeaderValueData(HeaderValueData& data, const booster::data::share::StringKeyLabel& headerValue, char separator) {

  booster::utils::parser::Caret caret(reinterpret_cast<const char*>(headerValue.getData()), headerValue.getSize());
