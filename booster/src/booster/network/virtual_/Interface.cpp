...
std::unordered_map<booster::String, std::weak_ptr<Interface>> Interface::m_registry;
...
void Interface::registerInterface(const std::shared_ptr<Interface>& _interface) {
  ...
  if(it != m_registry.end()) {
    ...
    ("[booster::network::virtual_::Interface::registerInterface()]: Error. Interface with such name already exists - '" + *_interface->getName() + "'.");
  }
...
void Interface::unregisterInterface(const booster::String& name) {
  ...
  if(it == m_registry.end()) {
    throw std::runtime_error
      ("[booster::network::virtual_::Interface::unregisterInterface()]: Error. Interface NOT FOUND - '" + *name + "'.");
  }
...
Interface::Interface(const booster::String& name)
  : m_name(name)
...
Interface::~Interface() {
...

  {
    ...
    if (m_listenerLock != nullptr) {
      BOOSTER_LOGe("[booster::network::virtual_::Interface::~Interface()]",
...
std::shared_ptr<Interface> Interface::obtainShared(const booster::String& name) {
...
std::shared_ptr<Interface::ListenerLock> Interface::bind() {
  ...
  throw std::runtime_error(
    "[booster::network::virtual_::Interface::bind()]: Can't bind to interface '" + *m_name + "'. Listener lock is already acquired");
}
...
void Interface::unbindListener(ListenerLock* listenerLock) {
  ...
  if(m_listenerLock != nullptr && m_listenerLock == listenerLock) {
    ...
    BOOSTER_LOGe("[booster::network::virtual_::Interface::unbindListener()]", "Error! Unbinding wrong listener!!!")
  }
}
