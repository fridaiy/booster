...
#ifndef booster_data_mapping_Tree_hpp
#define booster_data_mapping_Tree_hpp
...
public:
  ...
  operator T () const {
    ...
    throw std::runtime_error("[booster::data::mapping::Tree::operator T ()]: Value is NOT a Primitive type.");
  ...
  T getPrimitive() const {
    if(m_type != NodePrimitiveType<T>::type) {
      throw std::runtime_error(std::string("[booster::data::mapping::Tree::getPrimitive()]: NOT a ") + NodePrimitiveType<T>::name);
    }
    T result;
...
#endif //booster_data_mapping_Tree_hpp
